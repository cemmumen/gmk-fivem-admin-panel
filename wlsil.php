...
{
    $hex=$_GET["id"];
    $sorgu = $baglanti->query("DELETE FROM discordwlsistemi WHERE steam='".$hex."'");
    $sorgu2 = $baglanti->query("DELETE FROM whitelist WHERE identifier='".$hex."'");
...
while($sonucisim = $sorgu4->fetch_assoc())
{
    ...
    $sorgu3 = $baglanti->query("INSERT INTO panellog (admin, islem, zaman) VALUES ('".$_SESSION["user"]."', '".$ad." ".$soyad." isimli kişinin <b>whitelistini sildi.</b> Kişinin HEX numarası: ".$hex."', '".$zaman."')");
    header("Location:whitelist.php?w=1");
